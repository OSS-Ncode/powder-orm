...
async def _run():
    ...
    batch = await db.run(
        powder.Query.table("users").select("id", "name", "score").order("id", "ASC")
    )
    ...
    ids = batch.column("id")
    assert ids.type is powder.DataType.INT64
    assert ids.to_list() == [1, 2, 3]

    names = batch.column("name")
    assert names.type is powder.DataType.UTF8
    assert names.to_list() == ["alice", "bob", "héllo 🌍"]

    scores = batch.column("score")
    assert scores.type is powder.DataType.FLOAT64
