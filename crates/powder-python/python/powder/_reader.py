...
The native extension returns query results as PCB ``bytes``. This module parses
...
class Batch:
    ...
    def __repr__(self) -> str:
        ...
        return f"<powder.Batch rows={self.num_rows} [{cols}]>"


def decode_batch(buf: bytes) -> Batch:
    """Decode an PCB buffer into a :class:`Batch`."""
    mv = memoryview(buf)
    if mv[0:4] != _MAGIC:
        raise ValueError("not an PCB buffer (bad magic)")
    version = struct.unpack_from("<H", mv, 4)[0]
    if version != 1:
        raise ValueError(f"unsupported PCB version {version}")
    ...
    for c in range(num_columns):
        ...
        if dtype is DataType.INT64:
            ...
            raise ValueError(f"unsupported PCB type code {dtype}")
