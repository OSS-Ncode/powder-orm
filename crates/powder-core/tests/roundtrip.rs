use powder_core::array::{ColumnBuilder, ColumnData};
use powder_core::query::{Order, Query};
use powder_core::{Client, DataType, RecordBatch, Value};
...
async fn client_end_to_end() {
    ...
    // And the FFI entry point produces a decodable PCB buffer.
