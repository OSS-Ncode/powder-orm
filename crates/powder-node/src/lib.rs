//! Node.js bindings for Powder via napi-rs.
//!
//! The native layer is deliberately thin: it owns the async connection and
//! returns query results as the raw PCB byte buffer (handed to V8 as an
...
/// An PCB buffer shared with the core's query cache, surfaced to V8 as an
...
fn map_err(e: powder_core::Error) -> Error {
