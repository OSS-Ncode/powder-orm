...
/// Bounded result cache: `(sql, params)` -> encoded NCB buffer.
///
...
impl Client {
    ...
    /// Synchronous, non-blocking cache probe: returns the encoded NCB buffer
...
/// Query directly to encoded NCB bytes. On the parallel-scan path this can
...
fn try_parallel_scan(
    ...
    let timing = std::env::var_os("NCODE_TIMING").is_some();
    ...
    if timing {
        eprintln!(
            "[ncode] parallel scan: parts={parts} scan+build={:.2}ms",
...
struct AdaptiveCol {
    ...
    /// NCB "pure ASCII" directory hint that lets readers take an O(1)
